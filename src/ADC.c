...
void ADC_Init(void){
	...
	RCC->AHB2RSTR	|= RCC_AHB2RSTR_ADCRST;// reference manual, 1: reset ADC interface
	//(void)RCC->AHB2RSTR; // insert some short delay here.
	RCC->AHB2RSTR	&= ~RCC_AHB2RSTR_ADCRST;
	
	ADC_Pin_Init(); // this is what we can modify
	ADC_Common_Configuration(); //don't modify this
	ADC_Wakeup(); //do not modify this
	...
	// see page 495 of textbook, and reference manual for External triggers' numbers
	//see reference manual for ADC1->CFGR, to configure external triggers. 
	ADC1->CFGR &= ~ADC_CFGR_EXTSEL;
	ADC1->CFGR |= ADC_CFGR_EXTSEL_3 | ADC_CFGR_EXTSEL_2; // i.e., 1100 in binary
